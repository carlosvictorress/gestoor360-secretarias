...
from reportlab.platypus import BaseDocTemplate, Frame, PageTemplate, Image, Table, TableStyle, Paragraph, Spacer, HRFlowable
...
def alterar_licenca_command():
    ...
    with app.app_context():
        licenca = License.query.first()
        if not licenca:
            print("Nenhuma licença encontrada. Use 'flask init-license' para criar uma.")
            return
        ...
        try:
            ...
            print(f"A nova data de expiração é: {nova_data_expiracao.strftime('%d/%m/%Y')}")
        except ValueError:
...
def login():
# <--- Nível 1 de Indentação (Início da função)
    
    if request.method == "POST":
    # <--- Nível 2 de Indentação (Código DENTRO do 'if')
    ...
    # <--- Nível 1 de Indentação (Código FORA do 'if', para o GET)
    # Esta parte só executa quando a página é carregada pela primeira vez.
    secretarias_query = db.session.query(Servidor.lotacao).filter(Servidor.lotacao.isnot(None)).distinct().order_by(Servidor.lotacao).all()
    secretarias = [item[0] for item in secretarias_query]
...
def check_license(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        allowed_routes = ["login", "logout", "renovar_licenca", "admin_licenca", "static", "uploaded_file", "registrar_ponto"]
...
def serve_css(filename):
    return send_from_directory(os.path.join(basedir, "static", "css"), filename, mimetype="text/css")
...
def login():
    if request.method == "POST":
        # ... (A lógica do POST será ajustada no Passo 4) ...
        # ... por enquanto, deixe como está ...
    
    # --- LÓGICA PARA CARREGAR A PÁGINA (GET) ---
    # Busca todas as lotações/secretarias distintas e não nulas do cadastro de servidores
    secretarias_query = db.session.query(Servidor.lotacao).filter(Servidor.lotacao.isnot(None)).distinct().order_by(Servidor.lotacao).all()
...
def admin_licenca():
    ...
    # Esta linha espera encontrar o arquivo 'admin_licenca.html'
...
def lista_servidores():
    ...
    hoje = datetime.now().date()
    status_servidores = {}
    requerimentos_ativos = Requerimento.query.filter(Requerimento.status == "Aprovado", Requerimento.data_inicio_requerimento <= hoje).all()
    ...
    return render_template("index.html", servidores=servidores, funcoes_disponiveis=funcoes_disponiveis, lotacoes_disponiveis=lotacoes_disponiveis, status_servidores=status_servidores)
...
def delete_server(id):
    servidor = Servidor.query.get_or_404(id)
    dependencias = []
    if servidor.contratos: dependencias.append("contratos")
    # if servidor.abastecimentos: dependencias.append("registros de abastecimento")
    if servidor.requerimentos: dependencias.append("requerimentos")
    if servidor.pontos: dependencias.append("registros de ponto")
    if servidor.rotas_como_motorista: dependencias.append("rotas de transporte (como motorista)")
    if servidor.rotas_como_monitor: dependencias.append("rotas de transporte (como monitor)")
    ...
    if request.method == "POST":
        try:
            ...
            novo_req = Requerimento(
                autoridade_dirigida=request.form.get("autoridade_dirigida"), servidor_cpf=cpf_servidor, natureza=request.form.get("natureza"),
                natureza_outro=request.form.get("natureza_outro"),
                data_admissao=datetime.strptime(request.form.get("data_admissao"), "%Y-%m-%d").date() if request.form.get("data_admissao") else None,
                data_inicio_requerimento=datetime.strptime(request.form.get("data_inicio_requerimento"), "%Y-%m-%d").date(),
                duracao=request.form.get("duracao"), periodo_aquisitivo=request.form.get("periodo_aquisitivo"),
                informacoes_complementares=request.form.get("informacoes_complementares"), parecer_juridico=request.form.get("parecer_juridico"),
                status="Em Análise",
            )
            db.session.add(novo_req)
            db.session.commit()
            registrar_log(f"Criou novo requerimento (ID: {novo_req.id}) para o servidor {servidor.nome}.")
...
def editar_requerimento(req_id):
    ...
    if request.method == "POST":
        try:
            ...
            req.natureza_outro = request.form.get("natureza_outro") if req.natureza == "Outro" else None
            req.data_admissao = datetime.strptime(request.form.get("data_admissao"), "%Y-%m-%d").date() if request.form.get("data_admissao") else None
            req.data_inicio_requerimento = datetime.strptime(request.form.get("data_inicio_requerimento"), "%Y-%m-%d").date()
            req.duracao = request.form.get("duracao")
            req.periodo_aquisitivo = request.form.get("periodo_aquisitivo")
            req.informacoes_complementares = request.form.get("informacoes_complementares")
            req.parecer_juridico = request.form.get("parecer_juridico")
            db.session.commit()
            registrar_log(f"Editou o requerimento ID {req.id} para o servidor {req.servidor.nome}.")
...
def gerar_requerimento_pdf(req_id):
    ...
    buffer = io.BytesIO()
    doc = BaseDocTemplate(buffer, pagesize=A4, leftMargin=1.5*cm, rightMargin=1.5*cm, topMargin=4*cm, bottomMargin=2.5*cm)
    frame = Frame(doc.leftMargin, doc.bottomMargin, doc.width, doc.height, id='normal')
    template = PageTemplate(id='main_template', frames=[frame], onPage=lambda canvas, doc: cabecalho_e_rodape_moderno(canvas, doc, "Requerimento Funcional"))
    doc.addPageTemplates([template])
    styles = getSampleStyleSheet()
    label_style = ParagraphStyle(name='LabelHeader', fontName='Helvetica-Bold', fontSize=10, textColor=colors.white, backColor=colors.HexColor("#004d40"), alignment=TA_CENTER, paddingLeft=4, paddingTop=3, paddingBottom=3)
    data_style = ParagraphStyle(name='Data', fontName='Helvetica', fontSize=10, alignment=TA_LEFT, leftIndent=20, spaceBefore=6)
    story = []
    box_grid_style = TableStyle([('GRID', (0,0), (-1,-1), 1, colors.black), ('VALIGN', (0,0), (-1,-1), 'TOP'), ('LEFTPADDING', (0,0), (-1,-1), 0), ('RIGHTPADDING', (0,0), (-1,-1), 0), ('TOPPADDING', (0,0), (-1,-1), 0), ('BOTTOMPADDING', (0,0), (-1,-1), 2)])
    def criar_celula(label, data):
        return [Paragraph(label, label_style), Paragraph(data or " ", data_style)]
    tabela1 = Table([[criar_celula("AUTORIDADE A QUEM É DIRIGIDA", requerimento.autoridade_dirigida)]], colWidths=[18*cm])
    tabela2 = Table([[criar_celula("NOME COMPLETO DO(A) SERVIDOR(A)", servidor.nome)]], colWidths=[18*cm])
    data_nasc = servidor.data_nascimento.strftime("%d/%m/%Y") if servidor.data_nascimento else " "
    tabela3 = Table([[criar_celula("CARGO/FUNÇÃO", servidor.funcao), criar_celula("CLASSE/NÍVEL", servidor.classe_nivel), criar_celula("DATA DE NASCIMENTO", data_nasc)]], colWidths=[6*cm, 6*cm, 6*cm])
    data_adm = requerimento.data_admissao.strftime("%d/%m/%Y") if requerimento.data_admissao else " "
    tabela4 = Table([[criar_celula("DATA DE ADMISSÃO", data_adm), criar_celula("LOTAÇÃO", servidor.lotacao), criar_celula("Nº DE CONTRA CHEQUE", servidor.num_contra_cheque), criar_celula("TELEFONE", servidor.telefone)]], colWidths=[4.5*cm, 4.5*cm, 4.5*cm, 4.5*cm])
    tabela5 = Table([[criar_celula("LOCAL DE TRABALHO", servidor.local_trabalho), criar_celula("ENDEREÇO RESIDENCIAL", servidor.endereco)]], colWidths=[9*cm, 9*cm])
    natureza_req = requerimento.natureza_outro if requerimento.natureza == "Outro" else requerimento.natureza
    data_inicio_req = requerimento.data_inicio_requerimento.strftime("%d/%m/%Y") if requerimento.data_inicio_requerimento else " "
    tabela6 = Table([[criar_celula("NATUREZA DO REQUERIMENTO", natureza_req), criar_celula("INÍCIO", data_inicio_req), criar_celula("PERÍODO DE AQUISIÇÃO", requerimento.periodo_aquisitivo)]], colWidths=[9*cm, 4.5*cm, 4.5*cm])
    tabela7 = Table([[criar_celula("INFORMAÇÕES COMPLEMENTARES", requerimento.informacoes_complementares), criar_celula("PARECER JURÍDICO", requerimento.parecer_juridico)]], colWidths=[9*cm, 9*cm], rowHeights=4*cm)
    tabela8 = Table([[[Paragraph("ASSINATURA DO REQUERENTE", label_style)]]], colWidths=[18*cm], rowHeights=2*cm)
    tabela9 = Table([[[Paragraph("SETOR DE RECURSOS HUMANOS", label_style), Spacer(1, 1*cm), Paragraph("ASSINATURA E CARIMBO", styles['Normal'])], [Paragraph("CHEFE IMEDIATO DO SETOR", label_style), Spacer(1, 1*cm), Paragraph("ASSINATURA", styles['Normal'])]]], colWidths=[9*cm, 9*cm], rowHeights=2.5*cm)
    for t in [tabela1, tabela2, tabela3, tabela4, tabela5, tabela6, tabela7, tabela8, tabela9]:
        t.setStyle(box_grid_style)
        story.append(t)
        story.append(Spacer(1, 0.2*cm))
    liberado_check = "☐"
    nao_liberado_check = "☐"
    liberacao_style = ParagraphStyle(name="Liberacao", fontName="Helvetica", fontSize=10, alignment=TA_LEFT)
    tabela10 = Table([[Paragraph(f"{liberado_check} SERVIDOR LIBERADO", liberacao_style), Paragraph(f"{nao_liberado_check} SERVIDOR NÃO LIBERADO", liberacao_style)]], colWidths=[9*cm, 9*cm])
    story.append(tabela10)
    doc.build(story)
    buffer.seek(0)
    response = make_response(buffer.getvalue())
    response.headers["Content-Type"] = "application/pdf"
    nome_arquivo = f'Requerimento_{requerimento.id}_{servidor.nome.replace(" ", "_")}.pdf'
    response.headers["Content-Disposition"] = f"inline; filename={nome_arquivo}"
    registrar_log(f'Gerou o PDF do requerimento ID {req_id} para o servidor "{servidor.nome}".')
    return response
...
def get_servidor_data(cpf):
    ...
    if servidor.tipo_vinculo != "Servidor Efetivo":
        return jsonify({"error": f"Requerimentos disponíveis apenas para Servidores Efetivos. Vínculo atual: {servidor.tipo_vinculo}"}), 403
    servidor_data = { "nome": servidor.nome, "funcao": servidor.funcao, "classe_nivel": servidor.classe_nivel, "data_nascimento": servidor.data_nascimento.strftime("%d/%m/%Y") if servidor.data_nascimento else "", "data_admissao": servidor.data_inicio.strftime("%Y-%m-%d") if servidor.data_inicio else "", "lotacao": servidor.lotacao, "num_contra_cheque": servidor.num_contra_cheque, "telefone": servidor.telefone, "local_trabalho": servidor.local_trabalho, "endereco": servidor.endereco, }
...
def gerenciar_veiculos():
    if request.method == "POST":
        ...
        novo_veiculo = Veiculo(
            placa=nova_placa, modelo=request.form.get("modelo"), tipo=request.form.get("tipo"), ano_fabricacao=int(request.form.get("ano_fabricacao") or 0),
            ano_modelo=int(request.form.get("ano_modelo") or 0), orgao=request.form.get("orgao"), autorizacao_detran=request.form.get("autorizacao_detran") or None,
            validade_autorizacao=datetime.strptime(request.form.get("validade_autorizacao"), "%Y-%m-%d").date() if request.form.get("validade_autorizacao") else None,
            renavam=request.form.get("renavam") or None, certificado_tacografo=request.form.get("certificado_tacografo") or None,
            data_emissao_tacografo=datetime.strptime(request.form.get("data_emissao_tacografo"), "%Y-%m-%d").date() if request.form.get("data_emissao_tacografo") else None,
            validade_tacografo=datetime.strptime(request.form.get("validade_tacografo"), "%Y-%m-%d").date() if request.form.get("validade_tacografo") else None,
        )
    ...
    veiculos_com_alerta = Veiculo.query.filter(or_(and_(Veiculo.validade_tacografo != None, Veiculo.validade_tacografo <= data_limite), and_(Veiculo.validade_autorizacao != None, Veiculo.validade_autorizacao <= data_limite))).order_by(Veiculo.modelo).all()
    veiculos = Veiculo.query.order_by(Veiculo.modelo).all()
    return render_template("veiculos.html", veiculos=veiculos, veiculos_com_alerta=veiculos_com_alerta, hoje=hoje, data_limite=data_limite)
...
def excluir_veiculo(placa):
    ...
    try:
        ...
        flash("Veículo e seus registros de abastecimento foram excluídos com sucesso!", "success")
    except Exception as e:
        db.session.rollback()
...
def detalhes_veiculo(placa):
    ...
    return render_template("detalhes_veiculo.html", veiculo=veiculo, indicadores=indicadores, abastecimentos_com_analise=abastecimentos_com_analise, manutencoes=manutencoes, chart_labels=chart_labels, chart_consumo_data=chart_consumo_data, chart_custo_km_data=chart_custo_km_data)
...
def add_manutencao(placa):
    ...
    try:
        ...
        nova_manutencao = Manutencao(
            data=data, quilometragem=quilometragem, tipo_servico=request.form.get("tipo_servico"),
            custo=custo, descricao=request.form.get("descricao"), oficina=request.form.get("oficina"),
            veiculo_placa=placa,
        )
...
def excluir_manutencao(id):
    ...
    try:
        ...
        db.session.commit()
        registrar_log(f"Excluiu registro de manutenção ID {id} do veículo {placa_veiculo}.")
...
def lancar_abastecimento():
    if request.method == "POST":
        try:
            ...
            novo_abastecimento = Abastecimento(
                veiculo_placa=request.form.get("veiculo_placa"), motorista_id=request.form.get("motorista_id"),
                quilometragem=quilometragem_val, tipo_combustivel=request.form.get("tipo_combustivel"),
                litros=litros, valor_litro=valor_litro, valor_total=valor_total,
            )
            db.session.add(novo_abastecimento)
            db.session.commit()
            registrar_log(f"Lançou abastecimento de {litros}L para o veículo {novo_abastecimento.veiculo_placa}.")
            flash("Abastecimento registrado com sucesso!", "success")
            return redirect(url_for("lancar_abastecimento"))
        except (ValueError, TypeError):
            db.session.rollback()
            flash("Erro: valores numéricos inválidos (litros, valor, quilometragem). Verifique se os campos estão preenchidos corretamente.", "danger")
    ...
    return render_template("combustivel.html", veiculos=veiculos, motoristas=motoristas, abastecimentos=abastecimentos)
...
def get_servidor_details(num_contrato):
    ...
    if servidor:
        return jsonify({
            "nome": servidor.nome,
            "matricula": servidor.num_contrato, # Assumindo que matrícula é o num_contrato
...
# 2. ROTA PARA CRIAR UMA NOVA GUIA
...
def criar_gam():
    ...
    # Note que esta rota agora deve renderizar 'gam_form.html'
    # No final da função criar_gam
    return render_template("gam_form.html", servidores=servidores_efetivos, gam=None)


# 3. ROTA PARA EDITAR UMA GUIA EXISTENTE
@app.route("/gam/editar/<int:gam_id>", methods=["GET", "POST"])
@login_required
def editar_gam(gam_id):
    # Busca a guia específica ou retorna um erro 404 se não encontrar
    guia = GAM.query.get_or_404(gam_id)

    # Se o formulário estiver sendo enviado (método POST)
    if request.method == "POST":
        try:
            # Atualiza os dados do objeto 'guia' com as informações do formulário
    ...
    # Se a página estiver sendo apenas carregada (método GET)
    # Busca a lista de servidores para preencher o <select>
    servidores_efetivos = Servidor.query.filter(Servidor.tipo_vinculo.ilike("%efetivo%")).order_by(Servidor.nome).all()
    
    # Renderiza o formulário, passando a 'guia' com os dados e a lista de 'servidores'
    return render_template("gam_form.html", gam=guia, servidores=servidores_efetivos)

# 4. ROTA PARA EXCLUIR UMA GUIA
@app.route("/gam/excluir/<int:gam_id>")
@login_required
# @admin_required # Descomente esta linha se apenas admins puderem excluir
...
def gerar_pdf_gam(guia):
    """Gera o PDF da GAM com layout final, posicionando o título abaixo do timbre."""
    buffer = io.BytesIO()
    
    # --- AJUSTE 1: Aumentar a margem do topo para dar espaço ao cabeçalho completo ---
    doc = BaseDocTemplate(buffer, pagesize=A4, leftMargin=1.5*cm, rightMargin=1.5*cm, topMargin=4.5*cm, bottomMargin=1.5*cm)

    # --- FUNÇÃO INTERNA PARA DESENHAR O CABEÇALHO COMPLETO (Timbre + Títulos) ---
    def cabecalho_completo(canvas, doc):
        canvas.saveState()
        
        # 1. Desenha o Timbre (timbre.jpg) no topo
        try:
            ...
            if os.path.exists(timbre_path):
                ...
                # A altura da página A4 é A4[1]. Posiciona o timbre a 1.5cm do topo.
                logo.drawOn(canvas, 2*cm, A4[1] - 3*cm)
        except Exception as e:
            print(f"Erro ao carregar o timbre: {e}")

        # 2. Desenha o Texto ABAIXO do Timbre, em posições calculadas
        # A posição Y (vertical) é medida a partir da base da página.
        # A4[1] - 3.5*cm significa 3.5cm a partir do topo.
        posicao_y_texto = A4[1] - 3.7*cm
        
        # Define a fonte e o tamanho para o título e o desenha
        canvas.setFont('Helvetica-Bold', 12)
        # 10.5*cm é o centro horizontal de uma página A4 (21cm / 2)
        canvas.drawCentredString(10.5*cm, posicao_y_texto, "GUIA PARA ATENDIMENTO MÉDICO - GAM")
        
        # Redefine a fonte para o subtítulo e o desenha um pouco mais abaixo
        canvas.setFont('Helvetica-Bold', 11)
        canvas.drawCentredString(10.5*cm, posicao_y_texto - 0.5*cm, "PREFEITURA MUNICIPAL DE VALENÇA DO PIAUÍ")

        canvas.restoreState()

    # Configuração do template de página
    frame = Frame(doc.leftMargin, doc.bottomMargin, doc.width, doc.height, id='normal')
    ...
    # --- Estilos de Texto (não precisam mais dos estilos de título) ---
    ...
    # --- CONSTRUÇÃO DO CONTEÚDO PRINCIPAL (STORY) ---
    # ** O texto do título foi REMOVIDO daqui **
    
    # 2. Dados do Servidor
    dados_servidor_texto = f"<b>NOME DO SERVIDOR:</b> {guia.servidor.nome.upper()} &nbsp;&nbsp; <b>MATRÍCULA N.</b> {guia.servidor.num_contrato} &nbsp;&nbsp; <b>LOTAÇÃO:</b> {guia.servidor.lotacao}"
    
    # 3. Observações da Chefia (o resto do código continua igual)
    if guia.data_laudo:
        locale.setlocale(locale.LC_TIME, "pt_BR.UTF-8")
    ...
    texto_observacoes = (
        f"{guia.texto_inicial_observacoes or ''} Na declaração médica, datada do dia {data_laudo_formatada}, "
        f"o(a) médico(a) {guia.medico_laudo or '[médico]'} recomenda {guia.dias_afastamento_laudo or '[dias]'} dias de "
        f"afastamento das suas atividades laborais, pois declara que {guia.justificativa_laudo or '[justificativa]'}"
    )
    ...
    observacoes_data = [
        [Paragraph("OBSERVAÇÕES DA CHEFIA (ESPECIFICAR A DESCRIÇÃO DO CARGO/FUNÇÃO)", style_negrito)],
    ...
    assinatura_medico_data = [
        [Paragraph(medico_data_hora, style_corpo)], [Spacer(1, 0.5*cm)],
        [Paragraph("<b>RECOMENDAÇÕES DO MÉDICO</b>", style_corpo)], [Spacer(1, 2.5*cm)],
        [Paragraph("DATA: ______/______/__________", style_corpo)], [Spacer(1, 1*cm)],
        [HRFlowable(width="70%", thickness=0.5, color=colors.black, hAlign='CENTER')],
