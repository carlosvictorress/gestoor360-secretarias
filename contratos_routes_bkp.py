...
# ROTA PARA GUARDAR AS OPÇÕES DE ASSINATURA (ADICIONADA)
...
def visualizar_contrato_pdf(contrato_id):
    ...
    for linha in linhas:
        ...
        if linha_strip.startswith('<title>'):
            ...
            tabela.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'TOP'), ('LEFTPADDING', (0,0), (-1,-1), 0), ('RIGHTPADDING', (0,0), (-1,-1), 0)]))
            story.append(tabela)
    ...
    if contrato.assinatura_secretaria_tipo == 'imagem' and contrato.assinatura_secretaria_dados:
        ...
        if os.path.exists(caminho_assinatura):
            img = Image(caminho_assinatura, width=5*cm, height=2.5*cm)
            img.hAlign = 'CENTER'
            story.append(img)
            story.append(Spacer(1, -0.5*cm))
        else:
